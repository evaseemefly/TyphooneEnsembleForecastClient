...
export default class CaseListView extends Vue {
    ...
    diveideCaseListByStatus(
        ...
        listStatue.push(
            new StatueInfo(StatueEnum.RUNNING, 0, 2, 'far fa-caret-square-right', 'my-info')
        )
        listStatue.push(
            new StatueInfo(StatueEnum.COMPLETED, 0, 3, 'far fa-check-circle', 'my-succes')
        )
    ...
    loadCaseList(): void {
        ...
        // TODO:[*] 20-06-01 此处存在一个bug就是由于后台改为分页的方式查询，所以此处只会加载分页后的数据
        loadCaseListByUser(typeProduct, 0, 0, 0).then((res: AxiosResponse): void => {
            // console.log(res)
            if (res.status === 200) {
                // 获取返回的case list
