...
// vuex -> types
import { SET_GEO_COVERAGEID, SET_GEO_COVERAGETYPE } from '@/store/types'
// 历史栅格数据查询列表
@Component({})
export default class CoverageSearchForm extends Vue {
    // 选择的 wind or current
    coverageType = DEFAULT_SELECT_KEY
    // TODO:[*] 20-06-04 此种方式不可行
    // @Mutation(SET_GEO_COVERAGETYPE, { namespace: 'geo' })
    // @Action('setCoverageType', { namespace: 'geo' })
    ...
    */
    ...
    // @Action('setCoverageType', { namespace: 'geo' })
    @Mutation(SET_GEO_COVERAGETYPE, { namespace: 'geo' }) setCoverageType

    @Watch('dictType')
    onDictType(key: number): void {
        console.log('|CoverageSearchForm| -> 监听到 dictType 发生变化')
        this.setCoverageType(key)
    }
    ...
    onCoverageType(key: number): void {
        ...
        console.log('|CoverageSearchForm| -> 监听到 coverageType 中发生变化')
