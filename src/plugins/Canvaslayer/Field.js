...
export default class Field {
    ...
    getCells(stride = 1) {
        ...
        for (let j = 0; j < this.nRows; j = j + stride) {
            for (let i = 0; i < this.nCols; i = i + stride) {
                const [lon, lat] = this._lonLatAtIndexes(i, j)
                const center = L.latLng(lat, lon)
                const value = this._valueAtIndexes(i, j)
                const c = new Cell(center, value, this.cellXSize, this.cellYSize)
                cells.push(c) // <<
            }
        }
        return cells
    ...
    _getDecimalIndexes(lon, lat) {
        ...
        const i = (lon - this.xllCorner) / this.cellXSize
        const j = (this.yurCorner - lat) / this.cellYSize
        return [i, j]
    }
    ...
    valueAt(lon, lat) {
        ...
        const value = this._valueAtIndexes(ci, cj)
        if (this._inFilter) {
     ...
     */
    randomPosition(o = {}) {
        const i = (Math.random() * this.nCols) | 0
        const j = (Math.random() * this.nRows) | 0
     ...
     */
    _longitudeAtX(i) {
        const halfXPixel = this.cellXSize / 2.0
     ...
     */
    _latitudeAtY(j) {
        const halfYPixel = this.cellYSize / 2.0
